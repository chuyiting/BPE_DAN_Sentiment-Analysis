...
class WordEmbeddings:
    ...
    def get_initialized_embedding_layer(self, frozen=True):
        ...
        self.vectors: word vectors/embeddings should have shape (num_embeddings, embedding_dim)
        """
...
def create_indexer_from_data(sentiment_ex: List[SentimentExample]):
    sentences = [" ".join(ex.words) for ex in sentiment_ex]
...
def get_indexer_and_embedding(embeddings_file: str=None, infile: str=None, embedding_dim: int=-1, frozen=True, tokenizer:Tokenizer=None):
    """
    complicated dependency TODO refactor the arguments
    Returns:
        indexer: Indexer
        embedding: torch.nn.Embedding
        sentiment_ex: List[SentimentExample]
    """
    sentiment_ex = read_sentiment_examples(infile)
    if embeddings_file is None:
        if tokenizer is not None:
            indexer = None
            embedding = torch.nn.Embedding(tokenizer.get_vocab_size()+2, embedding_dim=embedding_dim)
        else:
            indexer = create_indexer_from_data(sentiment_ex)
            embedding = torch.nn.Embedding(len(indexer), embedding_dim=embedding_dim)
    else:
        wordEmbedding = read_word_embeddings(embeddings_file)
        indexer = wordEmbedding.word_indexer
        embedding = wordEmbedding.get_initialized_embedding_layer(frozen=frozen)
    
    return indexer, embedding, sentiment_ex
